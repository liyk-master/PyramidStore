...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeVideoContent(self):
        rsp = self.fetch("https://www.chrqj.com/")
        root = self.html(self.cleanText(rsp.text))
        aList = root.xpath("//div[@class='item']//div[@class='content-card']")
        videos = []
        for a in aList:
            name = a.xpath(".//div[@class='info-title-box']/div[@class='title']")[0].text
            pic = a.xpath(".//div[@class='card-img']/div/img/@srcset")[0]
            print(pic)
            exit()
            mark = a.xpath(".//div[@class='info-intro']/text()")[1]
            sid = a.xpath(".//a")[0].get("href")
            sid = self.regStr("/detail/(\\S+)",sid)
            videos.append({
                "vod_id": sid,
                "vod_name": name,
                "vod_pic": pic,
